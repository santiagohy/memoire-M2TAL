...
pattern = f'pattern {{X->Y; Y[upos=NOUN]; Y[{feature}]}}'
...
comb_yes = Counter([(lst['X.upos'], lst['Y.deprel']) for lst in lst_predictors if lst[f'Y.{feature}'] == pattern2test])
comb_no = Counter([(lst['X.upos'], lst['Y.deprel']) for lst in lst_predictors if lst[f'Y.{feature}'] != pattern2test])
...
with open(f"../results/noun-{pattern2test}-{feature}-{treebank.split('.')[0]}.txt", "w", encoding="utf-8") as f:
