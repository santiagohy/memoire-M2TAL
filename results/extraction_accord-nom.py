...
corpora = "corpora-gsd"
filename = 'accords_nom_nombre'
pattern_M1 = 'X->Y; X[Number]; Y[Number]; X[upos=NOUN]'
pattern_M2 = 'X.Number = Y.Number'
        ...
        for k, v in corpus[tpl.sent_id][token_id].items():
            if k not in ("form", "lemma"):
                if k == "deprel" and tpl._fields[1+i] == "X":
                    continue
                elif k == "deprel" and tpl._fields[1+i] == "Y":
                    lst.append(f"X-[{v}]->Y")
                elif k == "upos" and tpl._fields[1+i] == "X":
                    continue
                else:
                    lst.append(f"{tpl._fields[1+i]}[{k}={v}]")

    Y_Number = re.search(r"Y\[Number=(.+?)\]", str(lst)).group(1)
    X_Number = re.search(r"X\[Number=(.+?)\]", str(lst)).group(1)

    if Y_Number == X_Number:
        agreement = "agree"
    else:
        agreement = "disagree"
    nlst = [x for x in lst if "Number" not in x]

    combs = powerset(nlst)

    for c in combs:
        if c:
            patterns[c][agreement] += 1
...
with open(f"../results/{filename}-fisher-{treebank.split('.')[0]}.txt", "w", encoding="utf-8") as f:
